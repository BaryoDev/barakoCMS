...
The listing mechanism is a NuGet tag query, not a registry people submit to:

| Query against NuGet search | Result |
|---|---|
| `tags:barakocms-module` | 20 packages (26 Sept 2026) |

Anything anyone else publishes with the same tag appears in that list too, with its icon, download
...
- **Scope.** Whether barakocms.com absorbs the documentation currently in the repo, or only links
