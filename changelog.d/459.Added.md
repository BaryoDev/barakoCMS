- **Workflow runs are swept, and failures outlive successes.**
  ...
  consequence of the window: a run whose provider has been unreachable for a fortnight is still an
  email somebody is waiting for. Zero or less on either setting keeps that class forever, which is
