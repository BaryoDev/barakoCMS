- **`GET /api/contents/by-slug/{type}/{slug}` told a caller that an entry they could not read
  ...
  With duplicate slugs saved before uniqueness was enforced (#717), only the oldest row was checked,
  so a newer duplicate the caller could read was refused; the route now returns the oldest of the
  matching rows the caller can read.
