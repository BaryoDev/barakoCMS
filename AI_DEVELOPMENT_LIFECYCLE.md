...
flowchart TD
    ...
    E --> F[Try to break it on<br/>dev-playground.baryo.dev]
    F -- found a problem --> A
    ...
    I -- new version --> K[Publish NuGet + GitHub Packages<br/>Build Docker amd64 + arm64 :playground<br/>Promote playground.baryo.dev<br/>Announce Discord + org discussions]
...
unit tests, and the Playwright regression pack. A red check blocks the merge. This is the same gate
whether the author is a person or an agent.
...
5. **Deploy** — the same forced-command pattern promotes the full stack to playground.baryo.dev and
   verifies 200.
6. **Announce** — the release is posted to the org discussions and Discord. Best-effort: a failed
   announcement never turns a successful release red.
